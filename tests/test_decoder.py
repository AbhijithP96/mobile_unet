...
def test_decoder_output_shape():
    ...
    assert (
        tf.reduce_min(output) >= 0.0 and tf.reduce_max(output) <= 1.0
    ), "Output values not in [0, 1] range"


def test_decoder_different_filter_sizes():
    """Test decoder with different filter sizes."""
    filter_list = [(128, 64, 32, 16), (256, 128, 64, 32), (512, 256, 128, 64)]
...
def test_decoder_trainable_parameters():
    ...
    trainable_params = sum(
        [tf.size(var).numpy() for var in decoder.trainable_variables]
    )
