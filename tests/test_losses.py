...
def test_dice_coefficient_perfect_match():
    ...
    assert abs(dice_score.numpy() - 1.0) < 1e-6, \
...
def test_dice_coefficient_no_overlap():
    ...
    # Assert perfect match
    assert abs(dice_score.numpy() - 0.0) < 1e-6, \
...
def test_dice_loss_perfect_match():
    ...
    # Assert perfect match
    assert abs(dice_loss.numpy() - 0.0) < 1e-6, \
...
def test_dice_loss_gradients():
    ...
    with tf.GradientTape() as tape:
        y_pred = tf.Variable(tf.random.uniform((4,224,224,1), 0, 1, dtype=tf.float32))
        tape.watch(y_pred)
        loss_value = dice_loss(y_true, y_pred)
...
def test_focal_dice_combination():
    ...
    for focal_wt, dice_wt in weights:
        ...
        combined_loss = FocalDiceLoss(
            focal_weight=focal_wt,
            dice_weight=dice_wt
        )(y_true, y_pred)

        expected_loss = focal_wt * focal_loss_val + dice_wt * dice_loss_val

        assert abs(combined_loss.numpy() - expected_loss.numpy()) < 1e-6, \
